...
public class MyketIABProxyActivity extends Activity
{
	private static final String TAG = "[MyketIAB][Proxy]";
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		try
		{
			...
			MyketIABPlugin.instance().getIabHelper().launchPurchaseFlow(this, sku, itemType, RC_REQUEST, MyketIABPlugin.instance(), developerPayload);
		}
		catch (Exception e)
		{
			...
			if (MyketIABPlugin.instance().getIabHelper() == null)
			{
				Log.e(TAG, "FATAL ERROR: Plugin singleton helper is null. Aborting operation.");
			} 
			else 
			{
				MyketIABPlugin.instance().getIabHelper().handleActivityResult(RC_REQUEST, 0, null);
			}
			finish();
		}
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if (!MyketIABPlugin.instance().getIabHelper().handleActivityResult(requestCode, resultCode, data))
		{
